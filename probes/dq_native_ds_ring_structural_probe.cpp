...
__host__ __device__ __forceinline__ uint16_t ds_bits(int q, int krow) {
    return static_cast<uint16_t>(0x3000 + ((q & 15) << 4) + (krow & 31));
}
...
#pragma unroll
    for (int word = 0; word < kFragWords; ++word) {
        ...
        if (source_slot_to_dst(lane, word, group, q, dst_word)) {
            ...
            frag.scalar[word] = half_from_bits(ds_bits(q, krow));
        } else {
            frag.scalar[word] = half_from_bits(0);
        }
    }
    return frag;
}
...
dq_native_ds_ring_structural_probe_kernel(const __half* __restrict__ k_input,
...
float expected_half_sum(int q, int slot, bool high_half) {
    ...
    for (int group = 0; group < 4; ++group) {
        for (int word = word_begin; word < word_end; ++word) {
            ...
            sum += half_bits_to_float(ds_bits(q, krow));
        }
    }
    return sum;
}
...
bool check_mode(const std::vector<float>& out,
    ...
    std::printf("native_ds_ring_structural_%s errors=%d max_abs=%g pass=%d\n",
...
int main() {
    ...
    hipLaunchKernelGGL(dq_native_ds_ring_structural_probe_kernel, dim3(1),
    ...
    std::printf(
        "native_ds_ring_structural_result producer_done=%d publisher_done=%d "
