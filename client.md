...
- Rejected after raw2: raw3 page depth, consumer1 score-prefetch stagger,
  causal=true specialization, and score read batch2. All were correct/resource
  clean, but regressed ticks by more than 5%.

## Current Diagnosis

The kernel does have MMAC and the matrix path is not the primary missing piece.
The active limiter is packet ownership and topology:
...
Workbook sheet `51_structural_pivot` defines the next candidate:
...
This duplicates Q/dO loads, but it removes CTA-wide raw-page coupling and gives
producer1 recurring useful work. It must replace the canonical topology in
place; do not add another public phase or stacked kernel.
