...
- 2026-08-25 correction: raw ISA places that opcode in test-data setup before
  the writer. The opcode-clean HEAD1694 run executes all FP32 writer/read
  transport paths; only permutation-aware downstream semantics remain open.
...
- The historical f32 recheck stopped at `0xd38b5008` after WDRA removal. This
  is superseded: the opcode is pre-writer `v_pk_sub_u16`, and the corrected
  HEAD1694 run executes all FP32 writer/read transport paths. Canonical kernels
  remain unchanged while the semantic source ABI is open.
...
The native FP32 DS writer now executes on PMD HEAD1694. Raw disassembly proves
the historical `0xd38b5008` failure was pre-writer `v_pk_sub_u16` generated by
test-data setup. All four writer/read paths preserve a complete slot
permutation with bank0; a permutation-aware dense MMAC oracle is still needed.
