...
The historical runs stopped on `0xd38b5007/0xd38b5008`, but raw code-object
disassembly now proves `0xd38b5008` is this setup instruction:

```asm
v_pk_sub_u16 v8, v12, -4
```

It was generated by modulo-based probe input construction before the first DS
writer. The actual FP32 writer starts later and encodes as:

```asm
ds_write_matrix_format v[1:4], s8 element:3 row:1 col:1
// D9DE0000 08010309
```

After replacing the unrelated packed-U16 setup, PMD HEAD1694 executes four
FP32 writers and eight FP32 readers. The run has no invalid opcode, panic,
spill, scratch or LDS bank conflict, and all four writer/read combinations are
complete 256-slot permutations. Therefore there is no current evidence for a
PMD FP32-writer decode defect.

The semantic contract remains open: none of the four raw fragments is
lane-linear identity, and the current direct-fragment oracle does not replay
the measured slot permutation before the downstream MMAC. That is a layout
oracle/source-ABI task, not PMD-001.
...
shapes. The f32 writer is no longer blocked by PMD-001; the 2026-08-25
opcode-clean run executes transport, while permutation-aware MMAC semantics
remain open.
