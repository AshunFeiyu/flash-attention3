...
BUILD_DIR="${FUSED5_SYMMETRIC_N16_BUILD_DIR:-build/probes/fused5_symmetric_n16_ds}"
BIN="${BUILD_DIR}/fused5_symmetric_n16_ds_probe"
ASM="${BUILD_DIR}/fused5_symmetric_n16_ds_probe.asm"
RUN_ROOT="${FUSED5_SYMMETRIC_N16_RUN_ROOT:-${SHAOBO_RUN_ROOT}/layout_probes}"
RUN_DIR="${RUN_ROOT}/fused5_symmetric_n16_ds_$(date +%Y%m%d_%H%M%S)"
PMD_TIMEOUT="${FUSED5_SYMMETRIC_N16_PMD_TIMEOUT:-300}"

grep -Fq 'kConsumerVgprs = 176' probes/fused5_symmetric_n16_ds_probe.cpp
grep -Fq 'kLdsBytes == 32 * 1024' probes/fused5_symmetric_n16_ds_probe.cpp
grep -Fq 'trans_mismatches' probes/fused5_symmetric_n16_ds_probe.cpp
grep -Fq 'normal_mismatches' probes/fused5_symmetric_n16_ds_probe.cpp

mkdir -p "${BUILD_DIR}"
set +e
TARGET_GFX=946 \
BUILD_ASM=1 \
SRC=probes/fused5_symmetric_n16_ds_probe.cpp \
BUILD_DIR="${BUILD_DIR}" \
BIN="${BIN}" \
ASM="${ASM}" \
...
python3 scripts/check_symbol_metadata_gate.py \
  --asm "${ASM_ABS}" \
  --symbol-regex fused5_symmetric_n16_ds_probe_kernel \
...
awk '
  ...
  /permlane/ { permlane += 1 }
  END {
    printf("asm_gate trap=%d write_t1_alt0=%d read_trans=%d read_normal=%d mmac=%d resize=%d scalar_read=%d permute=%d permlane=%d\n",
           trap, write_t1_alt0, read_trans, read_normal, mmac, resize,
           scalar_read, permute, permlane)
    if (trap != 0 || write_t1_alt0 < 2 || read_trans < 8 ||
        read_normal < 2 || mmac < 32 || resize != 3 || scalar_read != 0 ||
...
semantic_pass="$(grep -c 'fused5_symmetric_n16_ds trans_mismatches=0 normal_mismatches=0 dq_mismatches=0 dk_mismatches=0 pressure_mismatches=0 pass=1' pmd_stdout.log || true)"
config_pass="$(grep -c 'fused5_symmetric_n16_ds config waves=12 roles=32/176/176 groups=2 owners=4' pmd_stdout.log || true)"
...
grep -E '^fused5_symmetric_n16_ds ' pmd_stdout.log | tee result.txt || true
printf 'fused5_symmetric_n16_ds_probe_status=%s pmd_status=%s semantic=%s config=%s panic=%s vgpr_warnings=%s stats=%s bank=%s run=%s\n' \
