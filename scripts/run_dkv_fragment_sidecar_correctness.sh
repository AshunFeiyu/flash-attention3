...
case_id="dkv_fragment_sidecar_correctness_$(date +%Y%m%d_%H%M%S)"
...
export S="\${S:-128}"
export D="\${D:-128}"
./build/fa3_bwd_wasp_clean --fragment-check=1 --B=\${B} --H=\${H} --S=\${S} --D=\${D}
EOF
chmod +x "${case_script}"
...
if grep -Eiq 'panic|Program aborted|core dumped|Aborted' "${stdout_log}"; then
  echo "PMD dKV fragment sidecar correctness detected model abort; see ${stdout_log}" >&2
  exit 1
fi

if ! grep -q 'fa3_bwd_dkv_fragment_sidecar status=success' "${stdout_log}"; then
  echo "PMD dKV fragment sidecar correctness did not report successful status" >&2
  exit 1
fi

if ! grep -q 'pass=1' "${stdout_log}"; then
  echo "PMD dKV fragment sidecar correctness failed numerical comparison" >&2
  exit 1
fi

echo "dKV fragment sidecar correctness m5out: ${case_dir}/m5out"
