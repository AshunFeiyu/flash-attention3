...
BUILD_DIR="${DKV_4C_LIFECYCLE_BUILD_DIR:-build/dkv_owner16_4c_lifecycle}"
BIN="${BUILD_DIR}/dkv_owner16_4c_lifecycle_probe"
ASM="${BUILD_DIR}/dkv_owner16_4c_lifecycle_probe.asm"
RUN_ROOT="${DKV_4C_RUN_ROOT:-/zys/shaobo_runs}"
RUN_DIR="${RUN_ROOT}/dkv_owner16_4c_lifecycle_probe_$(date +%Y%m%d_%H%M%S)"
PMD_TIMEOUT="${DKV_4C_PMD_TIMEOUT:-300}"
...
BUILD_ASM=1 \
SRC=probes/dkv_owner16_4c_lifecycle_probe.cpp \
BUILD_DIR="${BUILD_DIR}" \
BIN="${BIN}" \
ASM="${ASM}" \
./build.sh
...
python3 scripts/check_symbol_metadata_gate.py \
  --asm "${ASM}" \
  --symbol-regex dkv_owner16_4c_lifecycle_probe_kernel

awk '
  /matrix_load_32x(16|32)_b16/ && /bps/ && /lds/ { bps += 1 }
  /ds_read_matrix_format/ || /ds_read_matrix_trans_format/ { read += 1 }
  /s_abarrier_try_wait/ { wait += 1 }
  /s_abarrier_arrive/ { arrive += 1 }
  /s_set_vgpr_size 128/ { resize128 += 1 }
  /^[[:space:]]*s_trap([[:space:]]|$)/ { trap += 1 }
  END {
    printf("asm bps=%d ds_read_matrix=%d abarrier_wait=%d abarrier_arrive=%d resize128=%d trap=%d\n",
           bps, read, wait, arrive, resize128, trap)
    if (bps < 16 || read < 32 || wait < 8 || arrive < 8 ||
        resize128 != 4 || trap != 0) {
      exit 1
    }
  }
...
panic_lines="$(grep -ciE 'panic:|fatal:|not init or has been freed|ABARRIER.*ERROR' pmd_stdout.log || true)"
pass_lines="$(grep -c 'dkv_owner16_4c_lifecycle_probe .* pass=1' pmd_stdout.log || true)"
bank_conflicts="0"
...
if [[ "${pmd_status}" != "0" || "${panic_lines}" != "0" ||
  ...
  printf 'dkv_owner16_4c_lifecycle_probe_status=FAIL pmd_status=%s pass_lines=%s panic=%s bank=%s run=%s\n' \
...
grep 'dkv_owner16_4c_lifecycle_probe' pmd_stdout.log | tee result.txt
printf 'dkv_owner16_4c_lifecycle_probe_status=PASS bank=%s run=%s\n' \
