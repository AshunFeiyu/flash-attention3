...
TARGET_GFX=946 BUILD_ASM=1 COMPILE_ONLY=1 \
SRC=src/fused_bwd_dq_reduce.cpp BUILD_DIR="${BUILD_DIR}" \
...
LINK_OBJECTS="${BUILD_DIR}/fused_bwd_kernel.o ${BUILD_DIR}/fused_bwd_dq_reduce.o ${BUILD_DIR}/fused_bwd_correctness.o" \
...
if grep -Eq '^[[:space:]]+global_atomic' "${BUILD_DIR}/fused_bwd_kernel.asm"; then
  echo "fused compute unexpectedly contains a global atomic" >&2
  exit 1
fi
if grep -Eq '^[[:space:]]+(s_set_vgpr_size|v_mmac)' \
    "${BUILD_DIR}/fused_bwd_dq_reduce.asm"; then
  echo "plain dQ reduction unexpectedly contains WDRA resize or MMAC" >&2
  exit 1
fi
