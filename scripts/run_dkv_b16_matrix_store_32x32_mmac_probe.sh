...
BUILD_DIR="${MATRIX_STORE_32X32_MMAC_BUILD_DIR:-build/probes}"
BIN="${BUILD_DIR}/dkv_b16_matrix_store_32x32_mmac_probe"
ASM="${BUILD_DIR}/dkv_b16_matrix_store_32x32_mmac_probe.asm"
RUN_ROOT="${MATRIX_STORE_32X32_MMAC_RUN_ROOT:-/zys/sb/matrix_store_32x32_mmac_probe}"
RUN_DIR="${RUN_ROOT}/run_$(date +%Y%m%d_%H%M%S)"
PMD_TIMEOUT="${MATRIX_STORE_32X32_MMAC_PMD_TIMEOUT:-900}"

TARGET_GFX=946 BUILD_ASM=1 \
SRC=probes/dkv_b16_matrix_store_32x32_mmac_probe.cpp \
...
python3 scripts/check_symbol_metadata_gate.py \
  --asm "${ASM_ABS}" \
  --symbol-regex matrix_store_32x32_mmac_source_kernel \
...
semantic_pass="$(grep -c 'matrix_store_32x32_mmac_source_status=PASS' pmd_stdout.log || true)"
...
grep -E '^matrix_store_32x32_mmac' pmd_stdout.log | tee result.txt || true
printf 'matrix_store_32x32_mmac_probe status=%s pmd_status=%s panic=%s stats=%s bank=%s run=%s\n' \
