...
BIN="${BIN}" \
ASM="${ASM}" \
EXTRA_CXXFLAGS="-DPDS_PROBE_HIGH_PRESSURE=1 -DPDS_PROBE_SPLIT_OUTPUT_READERS=1 -DPDS_PROBE_ACCUM_F32X4=16 -DPDS_PROBE_READER_VGPRS=160" \
./build.sh
...
pass_lines="$(grep -cE 'sync=abarrier .*mismatches=0 .*pass=1' pmd_stdout.log)"
