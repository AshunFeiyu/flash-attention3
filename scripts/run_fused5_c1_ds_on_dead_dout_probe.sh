...
BUILD_DIR="${FUSED5_C1_DS_DEAD_DOUT_BUILD_DIR:-build/probes/fused5_c1_ds_on_dead_dout}"
BIN="${BUILD_DIR}/fused5_c1_ds_on_dead_dout_probe"
ASM="${BUILD_DIR}/fused5_c1_ds_on_dead_dout_probe.asm"
RUN_ROOT="${FUSED5_C1_DS_DEAD_DOUT_RUN_ROOT:-${SHAOBO_RUN_ROOT}/layout_probes}"
RUN_DIR="${RUN_ROOT}/fused5_c1_ds_on_dead_dout_$(date +%Y%m%d_%H%M%S)"
PMD_TIMEOUT="${FUSED5_C1_DS_DEAD_DOUT_PMD_TIMEOUT:-300}"
...
TARGET_GFX=946 \
BUILD_ASM=1 \
SRC=probes/fused5_c1_ds_on_dead_dout_probe.cpp \
BUILD_DIR="${BUILD_DIR}" \
BIN="${BIN}" \
ASM="${ASM}" \
...
python3 scripts/check_symbol_metadata_gate.py \
  --asm "${ASM_ABS}" \
  --symbol-regex fused5_c1_ds_on_dead_dout_probe_kernel \
...
awk '
  ...
  END {
    ...
    if (trap != 0 || mls < 8 || write_t1_alt0 < 8 || read_trans < 16 ||
        read_normal < 8 || mmac < 64 || resize != 4 || abar_init != 14 ||
        abar_inv != 14 || scalar_read != 0 || permute != 0 ||
        permlane != 0) exit 1
  }
' "${ASM_ABS}"
...
grep -E '^fused5_c1_ds_dead_dout ' pmd_stdout.log | tee result.txt || true
printf 'fused5_c1_ds_on_dead_dout_probe_status=%s pmd_status=%s semantic=%s config=%s panic=%s vgpr_warnings=%s stats=%s bank=%s run=%s\n' \
