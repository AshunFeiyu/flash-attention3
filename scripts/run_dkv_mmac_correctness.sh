...
elif [[ "${WAVES}" == "12" && "${CAUSAL_SKIP}" == "1" ]]; then
...
if ! grep -Eq 'fa3_bwd_dkv_mmac(12(_mq64(_semantic)?|_causal_skip|_overlay|_score_brick)?)?_correctness status=success' "${stdout_log}"; then
